...
INSERT OVERWRITE TABLE dim_sku_full PARTITION (dt = '20250625')
SELECT
  sku.id,
  sku.price,
  sku.sku_name,
...
LEFT JOIN ods_spu_info spu ON sku.spu_id = spu.id AND spu.ds = '20250625'
LEFT JOIN ods_base_category3 c3 ON sku.category3_id = c3.id AND c3.ds = '20250625'
LEFT JOIN ods_base_category2 c2 ON c3.category2_id = c2.id AND c2.ds = '20250625'
LEFT JOIN ods_base_category1 c1 ON c2.category1_id = c1.id AND c1.ds = '20250625'
LEFT JOIN ods_base_trademark tm ON sku.tm_id = tm.id AND tm.ds = '20250625'
LEFT JOIN (
  SELECT
    sku_id,
...
(
    ...
    TBLPROPERTIES ('orc.compress' = 'snappy');
insert overwrite table dim_coupon_full partition(dt='20250625')
SELECT
    ci.id,
    ci.coupon_name,
...
(
    ...
    TBLPROPERTIES ('orc.compress' = 'snappy');
insert overwrite table dim_activity_full partition(dt='20250625')
select
    rule.id,
    info.id,
    activity_name,
...
(
    ...
    TBLPROPERTIES ('orc.compress' = 'snappy');
insert overwrite table dim_province_full partition(dt='20250625')
select
    province.id,
    province.name,
    province.area_code,
...
(
    ...
    TBLPROPERTIES ('orc.compress' = 'snappy');
insert overwrite table dim_promotion_pos_full partition(dt='20250625')
select
    `id`,
    `pos_location`,
    `pos_type`,
