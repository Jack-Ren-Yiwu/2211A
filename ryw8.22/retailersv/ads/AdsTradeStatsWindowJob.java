...
import com.stream.common.utils.SplitMainLateProcessFunction;
...
//         tableEnv.toDataStream(origin).print("原始数据");
        ...
        Table orderStatsTable = tableEnv.sqlQuery(
                "SELECT \n" +
                        ...
                        "FROM TABLE(\n" +
                        " TUMBLE(TABLE dwd_order_detail_enriched, DESCRIPTOR(event_time), INTERVAL '10' SECOND))\n" +
                      //  "WHERE event_type = 'order'\n" +
                        "GROUP BY window_start, window_end, sku_id"
        );
        tableEnv.toDataStream(orderStatsTable).print("order_stats");

        // 支付窗口统计（使用 event_time + Watermark）
        ...
        // 退款窗口统计（使用 event_time + Watermark）
...
// ---------- 迟到测流标签 ----------
        OutputTag<Row> lateOrderTag = new OutputTag<Row>("late_order_stats"){};
        OutputTag<Row> latePaymentTag = new OutputTag<Row>("late_payment_stats"){};
        OutputTag<Row> lateRefundTag = new OutputTag<Row>("late_refund_stats"){};

// ---------- 原始主流 ----------
        DataStream<Row> orderStream = tableEnv.toDataStream(orderStatsTable);
        DataStream<Row> paymentStream = tableEnv.toDataStream(paymentStatsTable);
        DataStream<Row> refundStream = tableEnv.toDataStream(refundStatsTable);

// ---------- 包装 ProcessFunction 提取迟到流 ----------
        SingleOutputStreamOperator<Row> orderMain = orderStream.process(new SplitMainLateProcessFunction(lateOrderTag));
        SingleOutputStreamOperator<Row> paymentMain = paymentStream.process(new SplitMainLateProcessFunction(latePaymentTag));
        SingleOutputStreamOperator<Row> refundMain = refundStream.process(new SplitMainLateProcessFunction(lateRefundTag));

// ---------- 获取测流 ----------
        DataStream<Row> lateOrderStream = orderMain.getSideOutput(lateOrderTag);
        DataStream<Row> latePaymentStream = paymentMain.getSideOutput(latePaymentTag);
        DataStream<Row> lateRefundStream = refundMain.getSideOutput(lateRefundTag);

// ---------- 写入 Kafka ----------
// ------------------ 写入 Kafka，调用你自定义的工具类 ------------------
        lateOrderStream.map((MapFunction<Row, String>) JSON::toJSONString)
                .sinkTo(KafkaUtils.buildKafkaSink("cdh01:9092", "late_order_stats"));

        latePaymentStream.map((MapFunction<Row, String>) JSON::toJSONString)
                .sinkTo(KafkaUtils.buildKafkaSink("cdh01:9092", "late_payment_stats"));

        lateRefundStream.map((MapFunction<Row, String>) JSON::toJSONString)
                .sinkTo(KafkaUtils.buildKafkaSink("cdh01:9092", "late_refund_stats"));
//        tableEnv.executeSql(
//                "CREATE TABLE dwd_order_detail_enriched (\n" +
//                        "  sku_id STRING,\n" +
//                        "  user_id STRING,\n" +
