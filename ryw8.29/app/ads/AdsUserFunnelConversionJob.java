...
public class AdsUserFunnelConversionJob {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<AdsUserFunnelConversion> resultStream =
                tableEnv.toChangelogStream(appendOnlyTable)
                        ...
                        .filter(f -> f.getUserId() != null &&
                                f.getViewToPayRate() != null &&
                                f.getViewToPayRate() != 0.0 &&
                                f.getCartCnt() != 0 &&
                                f.getViewToCartRate() != 0.0 &&
                                f.getCartToPayRate() != 0.0)
                        .keyBy(f -> true)  // 所有数据同一个 key，保证全局限制
                        .process(new LimitProcessFunction<>(200))
                        .returns(Types.POJO(AdsUserFunnelConversion.class));  // 必加类型信息
