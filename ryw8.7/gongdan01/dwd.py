...
from pyspark.sql.functions import col, date_format, current_date, date_sub, lit, current_timestamp
from pyspark.sql.functions import floor, rand, col, date_format, current_date, date_sub
...
def create_and_load_dwd_with_dim():
    ...
    # 创建表（保持不变）
    spark.sql('''CREATE TABLE IF NOT EXISTS dwd_user_behavior_log (
        log_id STRING,
        user_id INT,
        session_id STRING,
        device_type STRING,
    ...
    spark.sql('''CREATE TABLE IF NOT EXISTS dwd_product_view_log (
        view_id STRING,
        user_id INT,
        product_id INT,
        product_name STRING,
    ...
    spark.sql('''CREATE TABLE IF NOT EXISTS dwd_favorite_log (
        fav_id STRING,
        user_id INT,
        product_id INT,
        product_name STRING,
    ...
    spark.sql('''CREATE TABLE IF NOT EXISTS dwd_cart_log (
        cart_id STRING,
        user_id INT,
        product_id INT,
        product_name STRING,
    ...
    spark.sql('''CREATE TABLE IF NOT EXISTS dwd_order_info (
        order_id STRING,
        user_id INT,
        store_id INT,
        store_name STRING,
    ...
    spark.sql('''CREATE TABLE IF NOT EXISTS dwd_order_item (
    ...
    spark.sql('''CREATE TABLE IF NOT EXISTS dwd_payment_info (
        payment_id STRING,
        order_id STRING,
        user_id INT,
        product_id INT,
    ...
    spark.sql('''CREATE TABLE IF NOT EXISTS dwd_refund_log (
        refund_id STRING,
        order_id STRING,
        product_id INT,
        product_name STRING,
    ...
    dim_product = spark.table("dim_product").select(
        ...
        col("store_id").alias("dp_store_id"),
        col("product_name")
    )
    ...
    # dwd_payment_info -- 只改这里，模拟 product_id 和 store_id
    ods_payment = spark.table("ods_payment_info")
    payment_detail = ods_payment.select(
        col("payment_id"),
        col("order_id"),
        col("user_id").cast("int"),
        (floor(rand() * 2000) + 1).cast("int").alias("product_id"),
        (floor(rand() * 300) + 1).cast("int").alias("store_id"),
        col("pay_time"),
        col("pay_amount"),
        col("device_type"),
        col("pay_channel"),
    ...
    payment_detail.write.mode("overwrite").partitionBy("dt").format("orc").saveAsTable("dwd_payment_info")

    # dwd_refund_log
    ods_refund = spark.table("ods_refund_log")
    refund_detail = ods_refund.join(dim_product, ods_refund.product_id == dim_product.dp_product_id, "left") \
        .join(dim_store, dim_product.dp_store_id == dim_store.ds_store_id, "left") \
        .select(
            col("refund_id"),
            col("order_id"),
            col("product_id").cast("int"),
            col("product_name"),
            dim_product.dp_store_id.alias("store_id"),
            col("user_id").cast("int"),
            col("refund_time"),
            col("refund_amount"),
            col("refund_type"),
    ...
    refund_detail.write.mode("overwrite").partitionBy("dt").format("orc").saveAsTable("dwd_refund_log")
